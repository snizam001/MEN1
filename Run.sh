...
Rscript /media/sheikh/Swift0/greenCUTRUN_pipeline/package/greenCUTRUN/rscripts/ann.R -m 100 -d Peaks/h3k4me3.ENCFF862LUQ.replicatedNarrowPeaks.bed,./Peaks/JunD_narrow.Clean.bed,./Peaks/MLL1_all.Clean.bed -c 30 -n h3k4me3,JunD,MLL1 -o Overlap.bed -i ./Peaks/MEN1_all.Clean.bed
...
pie(as.numeric(d[3,-c(1,2)]),col=c("#c0504d","#9bbb59","#4f81bd"),labels=c("AP1","ATF","NA"),main=expression("JunD"^"++" * " H3k4me3"^"--" * "&MLL1"^"--")) 
pie(as.numeric(d[4,-c(1,2)]) + as.numeric(d[5,-c(1,2)]) + as.numeric(d[7,-c(1,2)]), col=c("#c0504d","#9bbb59","#4f81bd"),labels=c("AP1","ATF","NA"),main=expression("JunD"^"--" * " H3k4me3"^"++" * "/MLL1"^"++")) 
pie(as.numeric(d[1,-c(1,2)]),col=c("#c0504d","#9bbb59","#4f81bd"),labels=c("AP1","ATF","NA"),main=expression("JunD"^"--" * " H3k4me3"^"--" * "MLL1"^"--"))
pie(as.numeric(d[2,-c(1,2)]),col=c("#c0504d","#9bbb59","#4f81bd"),labels=c("AP1","ATF","NA"),main=expression("JunD"^"++" * " H3k4me3"^"++" * "MLL1"^"++"))
#pie(as.numeric(d[4,-c(1,2)]),col=c("#c0504d","#9bbb59","#4f81bd"),labels=c("AP1","ATF","NA"),main=expression("JunD"^"--" * " H3k4me3"^"--" * "MLL1"^"++")) 
#pie(as.numeric(d[5,-c(1,2)]),col=c("#c0504d","#9bbb59","#4f81bd"),labels=c("AP1","ATF","NA"),main=expression("JunD"^"--" * " H3k4me3"^"++" * "MLL1"^"--"))
pie(as.numeric(d[6,-c(1,2)]),col=c("#c0504d","#9bbb59","#4f81bd"),labels=c("AP1","ATF","NA"),main=expression("JunD"^"++" * " H3k4me3"^"--" * "MLL1"^"++")) 
#pie(as.numeric(d[7,-c(1,2)]),col=c("#c0504d","#9bbb59","#4f81bd"),labels=c("AP1","ATF","NA"),main=expression("JunD"^"--" * " H3k4me3"^"++" * "MLL1"^"++"))
pie(as.numeric(d[8,-c(1,2)]),col=c("#c0504d","#9bbb59","#4f81bd"),labels=c("AP1","ATF","NA"),main=expression("JunD"^"++" * " H3k4me3"^"++" * "MLL1"^"--")) 
...
do
	...
	s_ct=$(fgrep read1 SpikeIn/$file\_ctrl.flagstat.txt  | cut -d " " -f1)
	norm=$(echo "1/(($s_ct/$h_ct)/($s_ex/$h_ex))" | bc -l)
	bamCompare -b1 Bamfiles/$file\_expr.selected.bam -b2 Bamfiles/$file\_ctrl.selected.bam -o bamcompare/$file.selected.bw -p 30 -bl /home/sheikh/Databases/EncodeBlackListedRegions/hg38-blacklist.v2.bed --scaleFactors 1:$norm --effectiveGenomeSize 2913022398
...
plotHeatmap -m $output.gz -o $output.jpeg --colorMap RdBu --dpi 300 --refPointLabel "" --heatmapWidth 1.5 --zMin -2 --zMax 5 --whatToShow "heatmap and colorbar"
...
computeMatrix reference-point -R $bedfiles -b 5000 -a 5000 --missingDataAsZero -bl /home/sheikh/Databases/EncodeBlackListedRegions/hg38-blacklist.v2.bed --smartLabels -p 35  -S $samples -o $output.gz --samplesLabel $name
plotHeatmap -m $output.gz -o $output.jpeg --colorMap RdBu --dpi 300 --refPointLabel "" --zMin -3 --zMax 20  --heatmapWidth 2 --whatToShow "heatmap and colorbar"

#-- Coverage around peaks of menin for MEN1 and menin mutations
#_______________________________________

#- tagDirectory of the selected reads ()
Samples	Spike	Total Reads	SelectedReads	SpikeIn/TotalReads	Ratio	Homer Reads
...
jpeg("barplotData.jpeg",unit="in",res=300,height=3,width=2.5)
plot(c(0.5,12),c(0,110),type="n",axes=F,xlab="", ylab= "Percentage of peaks retained")
grid()
box()
x <- barplot(d, beside = T, las=2, ylim = c(0, 110), col = c("#4f81bc","#c0504e"), add =T, border = "black", cex.axis = 0.7, cex = 0.7 )
legend("topright",col="black",pt.bg=c("#c0504e","#4f81bc"), legend=c(expression("JunD"^"++" * " H3k4me3&MLL1"^"--"),expression("JunD"^"--" * " H3k4me3/MLL1"^"++")),bty='n', pch = 22,cex = 0.4)
...
for m in R52G E408Q E255K E359K

do

annotatePeaks.pl DifferentialPeaks/MEN1-$m-junD_loss.txt hg38 -size 10000 -hist 1 -d TagDir/MEN1_Selected TagDir/MEN1-$m\_Selected TagDir/MEN1_ctrl TagDir/MEN1-$m\_ctrl > DifferentialPeaks/MEN1-$m\-junD_loss.coverage.txt
annotatePeaks.pl DifferentialPeaks/MEN1-$m-junD_noloss.txt hg38 -size 10000 -hist 1 -d TagDir/MEN1_Selected TagDir/MEN1-$m\_Selected TagDir/MEN1_ctrl TagDir/MEN1-$m\_ctrl > DifferentialPeaks/MEN1-$m\-junD_noloss.coverage.txt
annotatePeaks.pl DifferentialPeaks/MEN1-$m-onlyMLL1Histone_loss.txt hg38 -size 10000 -hist 1 -d TagDir/MEN1_Selected TagDir/MEN1-$m\_Selected TagDir/MEN1_ctrl TagDir/MEN1-$m\_ctrl > DifferentialPeaks/MEN1-$m\-onlyMLL1Histone_loss.coverage.txt
annotatePeaks.pl DifferentialPeaks/MEN1-$m-onlyMLL1Histone_noloss.txt hg38 -size 10000 -hist 1 -d TagDir/MEN1_Selected TagDir/MEN1-$m\_Selected TagDir/MEN1_ctrl TagDir/MEN1-$m\_ctrl > DifferentialPeaks/MEN1-$m\-onlyMLL1Histone_noloss.coverage.txt
...
sed 's/Distance/\#Distance/g' DifferentialPeaks/MEN1-$m\-junD_loss.coverage.txt -i
sed 's/Distance/\#Distance/g' DifferentialPeaks/MEN1-$m\-junD_noloss.coverage.txt -i
sed 's/Distance/\#Distance/g' DifferentialPeaks/MEN1-$m\-onlyMLL1Histone_loss.coverage.txt -i
sed 's/Distance/\#Distance/g'  DifferentialPeaks/MEN1-$m\-onlyMLL1Histone_noloss.coverage.txt -i
...
jpeg("coverage.jpeg",unit="in",res=300,height=2*2,width=1.5*4)
opar<-par(mfrow=c(2,4))
par(mar=c(2,2,2,2))
j1 = read.table("DifferentialPeaks/MEN1-E408Q-junD_loss.coverage.txt")
j2 = read.table("DifferentialPeaks/MEN1-E408Q-junD_noloss.coverage.txt")
plot(j1[,1],j1[,2],type="n",ylim=c(max(j1[,2])*-1,max(j1[,2])),main="E408Q")
grid()
lines(j1[,1],j1[,2],col="orange",lwd=2)
lines(j1[,1],j1[,8],col="orange",lty=3,lwd=2)
lines(j1[,1],j1[,5]*-1,col="red",lwd=2)
lines(j1[,1],j1[,11],col="red",lty=3,lwd=2)
lines(j2[,1],j2[,2],col="darkblue",lwd=2)
lines(j2[,1],j2[,8],col="darkblue",lty=3,lwd=2)
lines(j2[,1],j2[,5]*-1,col="black",lwd=2)
lines(j2[,1],j2[,11],col="black",lty=3,lwd=2)
#---
j1 = read.table("DifferentialPeaks/MEN1-R52G-junD_loss.coverage.txt")
j2 = read.table("DifferentialPeaks/MEN1-R52G-junD_noloss.coverage.txt")
plot(j1[,1],j1[,2],type="n",ylim=c(max(j1[,2])*-1,max(j1[,2])),main="R52G")
grid()
lines(j1[,1],j1[,2],col="orange",lwd=2)
lines(j1[,1],j1[,8],col="orange",lty=3,lwd=2)
lines(j1[,1],j1[,5]*-1,col="red",lwd=2)
...
lines(j2[,1],j2[,11],col="black",lty=3,lwd=2)
#-
j1 = read.table("DifferentialPeaks/MEN1-E255K-junD_loss.coverage.txt")
j2 = read.table("DifferentialPeaks/MEN1-E255K-junD_noloss.coverage.txt")
plot(j1[,1],j1[,2],type="n",ylim=c(max(j1[,2])*-1,max(j1[,2])),main="E255K")
grid()
lines(j1[,1],j1[,2],col="orange",lwd=2)
lines(j1[,1],j1[,8],col="orange",lty=3,lwd=2)
lines(j1[,1],j1[,5]*-1,col="red",lwd=2)
...
lines(j2[,1],j2[,11],col="black",lty=3,lwd=2)
#-
j1 = read.table("DifferentialPeaks/MEN1-E359K-junD_loss.coverage.txt")
j2 = read.table("DifferentialPeaks/MEN1-E359K-junD_noloss.coverage.txt")
plot(j1[,1],j1[,2],type="n",ylim=c(max(j1[,2])*-1,max(j1[,2])),main="E359K")
grid()
lines(j1[,1],j1[,2],col="orange",lwd=2)
lines(j1[,1],j1[,8],col="orange",lty=3,lwd=2)
lines(j1[,1],j1[,5]*-1,col="red",lwd=2)
...
#-----------------------------------------------------
j1 = read.table("DifferentialPeaks/MEN1-E408Q-onlyMLL1Histone_loss.coverage.txt")
j2 = read.table("DifferentialPeaks/MEN1-E408Q-onlyMLL1Histone_noloss.coverage.txt")
plot(j1[,1],j1[,2],type="n",ylim=c(max(j1[,2])*-1,max(j1[,2])),main="E408Q")
grid()
lines(j1[,1],j1[,2],col="orange",lwd=2)
lines(j1[,1],j1[,8],col="orange",lty=3,lwd=2)
...
lines(j2[,1],j2[,11],col="black",lty=3,lwd=2)
#-
j1 = read.table("DifferentialPeaks/MEN1-R52G-onlyMLL1Histone_loss.coverage.txt")
j2 = read.table("DifferentialPeaks/MEN1-R52G-onlyMLL1Histone_noloss.coverage.txt")
plot(j1[,1],j1[,2],type="n",ylim=c(max(j1[,2])*-1,max(j1[,2])),main="R52G")
grid()
lines(j1[,1],j1[,2],col="orange",lwd=2)
lines(j1[,1],j1[,8],col="orange",lty=3,lwd=2)
...
lines(j2[,1],j2[,11],col="black",lty=3,lwd=2)
#-
j1 = read.table("DifferentialPeaks/MEN1-E255K-onlyMLL1Histone_loss.coverage.txt")
j2 = read.table("DifferentialPeaks/MEN1-E255K-onlyMLL1Histone_noloss.coverage.txt")
plot(j1[,1],j1[,2],type="n",ylim=c(max(j1[,2])*-1,max(j1[,2])),main="E255K")
grid()
lines(j1[,1],j1[,2],col="orange",lwd=2)
lines(j1[,1],j1[,8],col="orange",lty=3,lwd=2)
...
lines(j2[,1],j2[,11],col="black",lty=3,lwd=2)
#-
j1 = read.table("DifferentialPeaks/MEN1-E359K-onlyMLL1Histone_loss.coverage.txt")
j2 = read.table("DifferentialPeaks/MEN1-E359K-onlyMLL1Histone_noloss.coverage.txt")
plot(j1[,1],j1[,2],type="n",ylim=c(max(j1[,2])*-1,max(j1[,2])),main="E359K")
grid()
lines(j1[,1],j1[,2],col="orange",lwd=2)
lines(j1[,1],j1[,8],col="orange",lty=3,lwd=2)
...
#-
jpeg("coverage2.jpeg",unit="in",res=300,height=4*2,width=3*4)
opar<-par(mfrow=c(2,4))
par(mar=c(2,2,2,2))
plot(j1[,1],j1[,2],type="n",ylim=c(max(j1[,2])*-1,max(j1[,2])),main="E359K")
legend("topright",col=c("orange","orange","red","red","darkblue","darkblue","black","black"),lty=c(1,3,1,3,1,3,1,3),bty="n",legend=c("MEN1(WT)(experiment)","MEN1(WT)(control)","MEN1(mutant)(experiment)","MEN1(mutant)(control)","MEN1(WT)(experiment)","MEN1(WT)(control)","MEN1(mutant)(experiment)","MEN1(mutant)(control)"),cex=1)
